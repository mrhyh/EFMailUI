...
- (void)setup {
    ...
    NSArray *views = @[_label, _nameLabel, _indicatorImageView, _lineView];
    ...
    .heightIs(20);
    [_nameLabel setSingleLineAutoResizeWithMaxWidth:SCREEN_WIDTH/2-100];
...
- (void)setRiskTypeModel:(RiskTypeModel *)riskTypeModel {
    
    if(riskTypeModel.integer > 0 && riskTypeModel.integer < 10) {
        _label.text = [NSString stringWithFormat:@"0%ld", (long)riskTypeModel.integer];
    }else {
        _label.text = [NSString stringWithFormat:@"%ld", (long)riskTypeModel.integer];
    }
