...
- (void)setupView{
    ...
    KYMHLabel * leftTopLB = [[KYMHLabel alloc]initWithTitle:@"热门标签" BaseSize:CGRectMake(10, 15, 100, 20) LabelColor:[UIColor clearColor] LabelFont:[_middle floatValue] LabelTitleColor:_textSecondColor TextAlignment:NSTextAlignmentLeft];
...
- (void)callBackAction:(EFViewControllerCallBackAction)action Result:(NetworkModel *)result{
    if (action == EFMallViewModelCallBackActionGetHotTags) {
        if (result.status == NetworkModelStatusTypeSuccess) {
            self.hotTags = [NSArray yy_modelArrayWithClass:[HotTagModel class] json:result.content];
            [self setupHotTagView];
        }else{
            [UIUtil alert:@"获取热门标签失败"];
        }
    }
}
