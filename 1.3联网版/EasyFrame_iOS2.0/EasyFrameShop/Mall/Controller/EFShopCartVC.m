...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [wsCell EFNumberAddWithBlock:^(NSInteger number) {
    ...
    if (action & EFMailShopCart_Items) {
        if ([result.jsonDict[@"status"] intValue] == 200) {
            ...
            for (int i=0; i<5; i++) {
                ...
                for (int j=0; j<3; j++) {
#pragma mark 多层模型手动初始化
                    ShopCartContentModel *model = [[ShopCartContentModel alloc] init];
                    model.productItem = [[Productitem alloc] init];
                    model.productItem.shopItem = [[Shopitem alloc] init];
                    
                    model.objectId = 12;
                    model.quantity = 20;
                    model.productItem.marketPrice = 20;
                    model.productItem.price = 123;
                    model.productItem.name = @"iPhone6s";
                    model.productItem.shopItem.name = [NSString stringWithFormat:@"店铺%d",i];
                    [shopArray addObject:model];
                }
                [_dataArray addObject:shopArray];
            }
